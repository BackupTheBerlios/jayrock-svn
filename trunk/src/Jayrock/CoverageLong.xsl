...
<!-- created by Yves Lorphelin, largely inspired by the nunitsumary.xsl (see nantcontrib.sourceforge.net)-->
	<xsl:template match="coverage">
		<html>
			<head>
                ...
                .assembly { font-size: normal;   font-weight: bold;}
                .class {font-size:normal; cursor: hand; color: #777;}
                .module { color: navy; font-size: small; }
                .method {cursor: hand; color: ; font-size: normal; font-weight: bold; }
                .subtitle { color: black; font-size: large; font-weight: bold; }
                .hdrcell  {font-size:smaller; background-color: #DDEEFF; }
                .datacell {font-size:smaller; background-color: #FFFFEE; text-align: right; }
                .hldatacell {font-size:smaller; background-color: #FFCCCC; text-align: right; }
                .detailProcent {  font-size: small; font-weight: bold; padding-top: 1px; padding-bottom: 1px; padding-left: 3px; padding-right: 3px;}
                </style>
	...
	<xsl:template name="module">
		<xsl:for-each select="//module">
			...
			<div class="assembly">
				<a name="#{generate-id($module)}">Module <xsl:value-of select="$module" /></a>
			</div>
	...
	<xsl:template name="Methods">
		...
		<xsl:for-each select="//method[(@class = $class) and (parent::module/@assembly=$module)]">
			...
			<table cellpadding="3" cellspacing="0" width="90%">
				<tr>
					<td width="45%" class='method'><xsl:attribute name="onclick">javascript:toggle(<xsl:value-of select="$methid" />)</xsl:attribute>
	...
	<xsl:template name="ClassSummaryDetail">
		...
		<td width="35%">
			<div class="assembly">
				<a href="#{generate-id($module)}"><xsl:value-of select="$module" /></a>
			</div>
		</td>
		<td width="65%">
			<xsl:call-template name="detailProcent">
	...
	<xsl:template name="header">
		...
		<table><tr>
				<td class="class"><a onClick="ExpandAll();">Expand</a></td> 
				<td> | </td>
				<td class="class"><a onClick="CollapseAll();">Collapse</a></td>
			</tr></table>
		<hr size="1" />
	</xsl:template>
